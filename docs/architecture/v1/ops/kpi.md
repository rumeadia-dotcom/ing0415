...
| **마이그레이션 단일 소스** | DDL · view 정의는 `apps/api/supabase/migrations/` 한 곳. debug / real 두 프로젝트에 동일 적용. | `platform.md` §5.3 |
...
| 시점 | 동작 | 책임 |
|---|---|---|
| 앱 마운트 + 인증된 세션 감지 | 새 `sessions` row insert (`started_at = now()`) | `apps/web/src/features/auth/hooks/useSessionLifecycle.ts` |
...
```ts
// apps/api/supabase/functions/kpi_retention/index.ts
...
### 9.3 v2 확장 시 어드민 SPA
- `apps/web/src/features/admin/kpi/` 도메인 신설.
...
- [ ] **클라이언트 헬퍼** — `apps/web/src/lib/kpi/track.ts` zod 검증 + best-effort insert. PR 에 이벤트 추가 시 화이트리스트 갱신.
