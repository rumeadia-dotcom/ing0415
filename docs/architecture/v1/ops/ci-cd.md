...
| 잡 (`name:`) | 의존 | 실행 조건 | 역할 |
|---|---|---|---|
| `Setup (scope detection)` | — | 항상 | `run_heavy`(이벤트 기반) + `app`/`sql`(경로 필터, `dorny/paths-filter`) output 산출 |
...
- 경로 필터: `app`(apps/web·tests·빌드설정·ci.yml) → build/e2e, `sql`(마이그·*.sql·ci.yml) → pgTAP.
