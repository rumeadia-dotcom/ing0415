...
|---|---|---|---|---|---|
| `pull_request` | `develop` | `ci.yml` | install / typecheck / lint / vitest / playwright(골든패스) / build(dev) | dev | X |
| `pull_request` | `main` | `ci.yml` | 동일 + release sanity (real env 변수 존재 검증, 실제 빌드는 X) | dev | X |
| `push` | `develop` | `ci.yml` | 동일 (브랜치 보호용 재실행) | dev | X |
...
jobs:
  verify:
    ...
    env:
      # dev 모드 빌드 sanity 용. 실제 시크릿 아님.
      VITE_APP_MODE: dev
      VITE_USE_MOCK: 'true'
      VITE_SUPABASE_URL: https://dev.placeholder.supabase.co
