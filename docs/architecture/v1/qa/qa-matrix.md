...
| QA-AUTH-006 | 셀러 B 가 셀러 A 리소스 직접 접근 (RLS) | — | — | — | — | **미커버** | `testing.md` §7.2 의 RLS-SQL 단위 테스트 부재 — 핵심 P0 |
...
**§2 갭 요약**: 11번가 어댑터 전체 BLOCKED, RLS 단위 SQL 부재 (QA-AUTH-006), 429 백오프 (QA-MKT-ERR-002), 계정 상태 변경 알림 (QA-MKT-CONN-005), 자격증명 백업/복구 (QA-MKT-SEC-002).
...
| QA-HIST-006 | 셀러 격리 (RLS) — 다른 셀러 이력 접근 불가 | — | — | — | — | **미커버** | RLS-SQL 부재 |

**§4 갭 요약**: 알림 UI (QA-ERR-002/003/004), 오류 통계 차트 (QA-HIST-004), CSV export (QA-HIST-005), RLS 단위 (QA-HIST-006), Realtime E2E.
...
| QA-ORD-011 | 셀러 격리 (RLS) — 다른 셀러 주문 접근 불가 | — | — | — | — | **미커버** | RLS-SQL 부재 |
...
| QA-SHIP-013 | shipping_jobs / shipping_job_results 셀러 격리 (RLS) | — | — | — | — | **미커버** | RLS-SQL 부재 |
...
| QA-CFG-006 | logen_credentials `user_id_enc` / `cust_cd_enc` 평문 저장 부재 | — | — | — | release grep | **미커버 (RLS-SQL + 평문 grep CI)** | `pgcrypto` 적용 확인 |
...
| **권한 누수 (RLS)** | QA-FAIL-008 | — | **미커버** — 모든 셀러 테이블 RLS-SQL 단위 부재 (P0 critical) |
...
1. **RLS-SQL 단위 테스트 전면 부재** (QA-AUTH-006 / QA-HIST-006 / QA-ORD-011 / QA-SHIP-013) — P0 보안. `testing.md` §7.2 양식대로 두 셀러 JWT 로 cross-access 시도, 0 rows 또는 에러 확인. 모든 셀러 데이터 테이블 (8개+) 커버 필요.
