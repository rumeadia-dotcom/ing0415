...
| **11번가** (API Key) | `tests/unit/adapters/11st/parity.spec.ts` | N/A (API Key 무 refresh) | `parity.spec.ts` | `parity.spec.ts` | `parity.spec.ts` | **미작성** (발급 키 실호출 검증 권장) |
...
| QA-MKT-11ST-001 | 11번가 연결 (API Key) | 위 표 | `useConnectMarket.test.tsx` | — | release sandbox 수동 | E2E + 발급 키 실호출 | real 어댑터 구현 완료. apiCode / XML 엘리먼트 실호출 최종 검증 권장 |
...
**§2 갭 요약**: 11번가 real 어댑터 구현 완료 (발급 키 실호출 검증 권장), 429 백오프 (QA-MKT-ERR-002), 계정 상태 변경 알림 (QA-MKT-CONN-005), 자격증명 백업/복구 (QA-MKT-SEC-002). ~~RLS 단위 SQL 부재 (QA-AUTH-006)~~ → CI `pgtap-rls` 통합 후 커버.
...
| 기능 ID | 시나리오 | 단위 | 통합 | E2E | 수동 | 미커버 | 비고 |
|---|---|---|---|---|---|---|---|
| QA-ORD-001 | §6.1 10분 주기 5마켓 주문 수집 (pg_cron + orders-sync Edge Function) | `apps/api/supabase/functions/orders-sync/__tests__/sync.test.ts` | `tests/integration/orders-sync.test.ts` | `tests/e2e/orders.spec.ts` | — | — | |
...
| QA-ORD-006 | 11번가 주문 조회 (Open API `GetOrderList` ordStat 101) | `tests/unit/adapters/11st/parity.spec.ts` | `orders-sync/__tests__/sync.test.ts` | `orders.spec.ts` | — | 발급 키 실호출 검증 | real 어댑터 구현 완료 |
...
- **11번가 발급 키 실호출 검증** (QA-MKT-11ST-001 / QA-ORD-006) — real 어댑터 (5메서드 + fetchOrders / submitTracking) 구현 완료. 정확한 apiCode / 요청·응답 XML 엘리먼트명은 셀러 발급 키 실호출로 최종 검증 권장 (개발자포털 IP 화이트리스트 등록 후).
...
| 3 | 11번가 어댑터 발급 키 실호출 검증 (구현 완료) | P1 v1 범위 충족 | 외부 의존 (발급 키) |
