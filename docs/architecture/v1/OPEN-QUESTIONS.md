...
| OQ-10 | v1 활성 마켓 & 마켓별 인증 방식 | **v1 정식 = 네이버 / 쿠팡 / G마켓 / 옥션 / 11번가 5개 전부 (real 어댑터까지 동작, 2026-05-22 5마켓 확장 결정)**. 어댑터 인터페이스 = `AuthInput` 4-way discriminated union (`oauth_code` \| `hmac_key` \| `esm_jwt` \| `api_key`). credential 저장 = `credential_payload jsonb` 단일 컬럼 + pgcrypto 암호화. `refreshToken` = OAuth(네이버)만 사용 optional. 네이버 OAuth endpoint = `https://api.commerce.naver.com/external/v1/oauth2/token` (form-urlencoded, type=SELF). G마켓·옥션 = ESM Trading API (sa.esmplus.com, site 'G'/'A' 분기). 쿠팡 = HMAC-SHA256 영구 키. 11번가 = API Key. **2026-05-23 정정**: 5개 마켓 전부 가 셀러 키 발급 단계에서 IP 화이트리스트 등록을 요구 (이전엔 11번가만 있다고 기재). AWS Lightsail Market Gateway 의 고정 IP 1개를 5개 마켓 셀러 콘솔에 모두 등록하는 방식으로 일괄 해소 (`docs/architecture/v1/cross-cutting/market-gateway.md`). | 사실 확인: 5개 마켓 중 네이버만 OAuth. 쿠팡 = HMAC, G마켓·옥션 = ESM 자체 JWT, 11번가 = API Key. 어댑터 인터페이스를 v1 에서 4-way union 으로 확장하여 5개 마켓 동시 출시. 운영 적용 시 IP 등록 요구가 5개 마켓 공통임이 확인됨 (2026-05-23). | user + architect + backend / 2026-05-19 (5마켓 결정 2026-05-22 / IP 화이트리스트 5마켓 공통 정정 2026-05-23) |
