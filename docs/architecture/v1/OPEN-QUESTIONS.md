...
| OQ-10 | v1 활성 마켓 & 마켓별 인증 방식 | **v1 정식 = 네이버 / 쿠팡 / G마켓 / 옥션 4개 (real 어댑터까지 동작). 11번가 = v2 (IP 화이트리스트)**. 어댑터 인터페이스 = `AuthInput` 4-way discriminated union (`oauth_code` \| `hmac_key` \| `esm_jwt` \| `api_key`). credential 저장 = `credential_payload jsonb` 단일 컬럼 + pgcrypto 암호화. `refreshToken` = OAuth(네이버)만 사용 optional. 네이버 OAuth endpoint = `https://api.commerce.naver.com/external/v1/oauth2/token` (form-urlencoded, type=SELF). G마켓·옥션 = ESM Trading API (etapi.gmarket.com, site 'G'/'A' 분기). 쿠팡 = HMAC-SHA256 영구 키. 11번가 = API Key + IP 화이트리스트 → Edge Function outbound IP 동적 문제로 v2 이관 | 사실 확인: 5개 마켓 중 네이버만 OAuth. 쿠팡 = HMAC, G마켓·옥션 = ESM 자체 JWT, 11번가 = API Key. 어댑터 인터페이스를 v2 가 아닌 v1 에서 4-way union 으로 확장하여 4개 마켓 v1 출시. 11번가는 Supabase Pro 고정 IP / 외부 프록시 / 11번가 해제 신청 중 결정 후 v2 진입 | user + architect + backend / 2026-05-19 (갱신: 5마켓 MVP 확장 결정) |
