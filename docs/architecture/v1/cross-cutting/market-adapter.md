...
- **반환 타입**: `RegistrationFieldMeta`(zod 단일 소스 `apps/web/src/lib/schemas/esm.ts` §4.6 / Edge 미러 `_shared/schemas.ts`).
  필드: `key` / `label`(i18n key) / `kind`(`select`|`text`|`number`|`officialNotice`|`shippingProfile`) / `required` / `optionsSource?` / `helpText?` / `blockingReason?`(미입력 시 3단계 다음 버튼 tooltip).

> ⚠ **2026-05-30 갱신 (Layer 2 조회형 단일화 — `shipping-fee-model.md §2`)**: 배송 리소스 select 의 `optionsSource` 가 "우리 테이블"(`shippingProfiles`, ESM 생성형 전용) → **마켓별 조회형 lookup** 으로 바뀐다. 신규 값: `esmShippingLookup`(ESM `/shipping/places`·`/dispatch-policies` GET) / `elevenStOutbound`·`elevenStReturn`(11번가 `outboundarea`·반품지 GET) / 쿠팡·네이버 lookup. `shippingProfiles` 는 ESM 생성형과 함께 deprecate(`esm.md` 전환 결정 절). 또한 **11번가도 `getRegistrationFields` 를 정의**한다(출고지/반품지 select + officialNotice) — 아래 "메서드 생략" 은 naver/coupang 한정으로 축소.
