...
| **도메인** | `<ip-hyphen>.sslip.io` (예: `43-201-83-78.sslip.io`) | sslip.io 가 Static IP → 도메인 자동 매핑. DNS 패널 조작·도메인 구매 불요. Caddy 가 Let's Encrypt 자동 발급. 채택 이유: 운영 도메인 미보유 + 게이트웨이는 사용자 노출 0 (Edge Function 내부 통신) |
