...
> v1 의 **핵심 가치 동선**이며, 본 문서가 architect + security + backend + frontend + qa 모두 승인되기 전에는 `src/features/registration/*` 및 `supabase/functions/registration-*` 구현 PR 을 머지하지 않는다.
>
...
- **포함**:
  ...
  - 클라이언트 zod 스키마 단일 소스: `src/lib/schemas/registration.ts`.
- **제외 (다른 문서)**:
...
1. **클라이언트 Step 4 미리보기** — 클라이언트가 `src/lib/markets/<id>/transform.ts` 의 동일 함수를 import 해서 호출. 마켓별 변환 결과 카드를 그린다. 클라이언트는 외부 마켓 API 를 호출하지 않으므로 transformProduct 만 사용 가능.
...
```ts
// supabase/functions/registration-validate/index.ts
import { z } from 'zod';
...
```ts
// src/features/registration/hooks/useRegistrationJobRealtime.ts
...
> 위치: `src/lib/schemas/registration.ts`. RHF + Supabase insert + Edge Function 요청 / 응답 검증 모두 본 스키마 재사용.
>
> **본 zod 스키마는 `src/lib/schemas/registration.ts` 단일 소스.** 다른 features 문서(dashboard.md, history.md 등) 는 정의를 재선언하지 않고 본 모듈에서 `import` 만 한다. ENUM 값 변경은 본 문서 + `cross-cutting/registration-job-state.md` §3.1 동시 갱신.
...
| 오류 펼치기 | `src/components/ui/error-message.tsx` (`<ErrorMessage>` 접기 기본, CLAUDE.md 강제) |
...
추가 마켓 고유 코드는 어댑터별 매핑 테이블 (`src/lib/markets/<id>/errors.ts`) 에서 한국어 메시지로 변환 후 `errorMessage` 로 적재.
...
```ts
// src/features/registration/lib/categoryAutoMatch.ts
...
| 9 | 단일 zod 소스 (`src/lib/schemas/registration.ts`) 의 Step 별 스키마 | §9 | ☐ |
...
> **본 문서는 v1 의 가장 두꺼운 산출물이며, architect + security + backend + frontend + qa 5자 승인 전까지 `src/features/registration/*` 및 `supabase/functions/registration-*` 구현 PR 은 머지되지 않는다.**
