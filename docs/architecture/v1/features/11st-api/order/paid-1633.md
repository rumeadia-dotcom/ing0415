# Seller - 주문 - 주문번호별 상태조회

> 출처: https://openapi.11st.co.kr/openapi/OpenApiGuide.tmall?categoryNo=110&apiSeq=1633&apiSpecType=1
> categoryNo: `110` · apiSeq: `1633` · 섹션: 주문 > 결제완료
...
해당주문의 주문정보를 조회할수 있습니다.

| 항목 | 값 |
|---|---|
| Method | `GET` |
| URL | `https://api.11st.co.kr/rest/ordservices/complete/[ordNo]` |
| Protocol | https |
| Version | 1.6 |
| 응답 형식 | xml |
...
|---|---|---|---|---|---|---|
| `ordNo` | 주문번호 | string | Y |  | 201406011234567 |  |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`ordPayAmtPerSeq` | 순번별 결제금액 | integer |  |  | 19000 |  |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`sellerDscPrcPerSeq` | 순번별 판매자 할인금액 | integer |  |  | 2280 |  |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`tmallDscPrcPerSeq` | 순번별 11번가 할인금액 | integer |  |  | 410 |  |
...
|---|---|---|---|---|---|---|
| `ns2:orders` | ns2:orders | object | Y |  |  |  |
| &nbsp;&nbsp;`ns2:result_code` | 결과코드 | enum | Y |  | 0 | (코드값: 0=조회된 결과가 없습니다.<br>설명 - 조회된 결과값이 없을 경우입니다. 에러가 아닙니다., -1=ex)주문/클레임 조회 오류 MSG : OpenAPI Key 에 해당하는 유저가 없습니다.<br>비지니스 Error. 예외적으로 발생되는 모든 에러. 메시지는 일정하지 않습니다., -1000=서버 점검중입니다.<br>설명 - 매주 금요일 새벽은 정기점검일입니다. 서버 차단이 있을수 있습니다.) |
