# 발송기한경과 요청내역 (배송준비중 목록조회)

> 출처: https://openapi.11st.co.kr/openapi/OpenApiGuide.tmall?categoryNo=111&apiSeq=6744&apiSpecType=1
> categoryNo: `111` · apiSeq: `6744` · 섹션: 주문 > 발주처리
...
조회기간은 당일기준 발송처리기한 경과된 주문건 모두 조회되며, 설정한 발송예정일 기준에 발송처리를 하지 못한 주문건입니다. (발송예정일 미설정 상품은 일괄 주문 후 2일내 발송으로 인지)
최대한 빨리 발송처리를 진행해주시고 만약 빠른 발송처리가 어렵다면 고객에게 발송지연안내를 해주시거나 품절취소를 진행해주세요.
발송기한 경과된 주문은 판매자 동의없이 고객직권 취소가 가능함을 안내드립니다.

| 항목 | 값 |
|---|---|
| Method | `GET` |
| URL | `https://api.11st.co.kr/rest/ordservices/delaydelivery/packagings` |
| Protocol | https |
| Version | 1.0 |
| 응답 형식 | xml |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`appmtselStockCd` | 발송마감일자 | string | Y | 500 |  | 2017/04/20 부터 신규제공 |
