...
> 출처: https://openapi.11st.co.kr/openapi/OpenApiGuide.tmall?categoryNo=110&apiSeq=6743&apiSpecType=1
> categoryNo: `110` · apiSeq: `6743` · 섹션: 주문 > 결제완료
...
* 중요 : 발주확인할 내역 (기간별 결제완료 목록조회) API와 컬럼값이 동일합니다. 맨아래 2개 컬럼값 (dlvSndDue 발송마감일자, delaySendDt 발송예정일자)만 추가되었으니 개발하실때 참고부탁드립니다.
조회기간은 당일기준 발송처리기한 경과된 주문건 모두 조회됩니다.
설정한 발송예정일 기준에 발송처리를 하지 못한 주문건입니다. (발송예정일 미설정 상품은 일괄 주문 후 2일내 발송으로 인지)
...
|---|---|
| Method | `GET` |
| URL | `https://api.11st.co.kr/rest/ordservices/delaydelivery/completes` |
| Protocol | https |
| Version | 1.1 |
| 응답 형식 | xml |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`dlvCst` | 배송비 | integer | Y | 12 | 0 | 2014/12/19(금) 주문건부터 신규제공 |
| &nbsp;&nbsp;&nbsp;&nbsp;`dlvCstType` | 배송비 착불 여부 | enum | Y | 5 | 03 | 2014/12/19(금) 주문건부터 신규제공 (코드값: 01=선불, 02=착불, 03=무료) |
| &nbsp;&nbsp;&nbsp;&nbsp;`bmDlvCst` | 도서산간배송비 | integer | Y | 12 | 4500 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`bmDlvCstType` | 도서산간배송비 착불 여부 | enum | Y | 5 | 04 | (코드값: 01=선불, 02=착불, 03=도서산간배송비 청구 필요 (선물하기 주문)) |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`delaySendDt` | 예상발송일 | string | Y |  | 2019-05-20 00:00:00 | 2019/05/20부터 신규제공 |
