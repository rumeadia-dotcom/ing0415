# Seller - 주문 - 결제대기 내역 (입금대기 목록조회)

> 출처: https://openapi.11st.co.kr/openapi/OpenApiGuide.tmall?categoryNo=110&apiSeq=1728&apiSpecType=1
> categoryNo: `110` · apiSeq: `1728` · 섹션: 주문 > 결제완료
...
무통장 입금 주문이 완료되어 결제를 기다리는 주문상품입니다.(예약상품 무통장 입금 주문건 포함)

| 항목 | 값 |
|---|---|
| Method | `GET` |
| URL | `https://api.11st.co.kr/rest/ordservices/standby/[startTime]/[endTime]` |
| Protocol | https |
| Version | 1.1 |
| 응답 형식 | xml |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`ordPrdStat` | 주문상태코드 | string | Y | 5 | 102 |  |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`bmDlvCst` | 도서산간배송비 | integer | Y |  | 0 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`bmDlvCstType` | 도서산간배송비 착불 여부 | enum | Y | 5 | 01 | 2014/12/19(금) 주문건부터 신규제공 (코드값: 01=선불, 02=착불, 04=도서산간배송비 청구 필요 (선물하기 주문)) |
| &nbsp;&nbsp;&nbsp;&nbsp;`dlvEtprsCd` | 배송업체코드 | string | Y | 8 | 00007 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`dlvMthdCd` | 배송방식 | enum | Y | 2 | 01 | 2015/03/20(금)부터 신규제공 (코드값: 01=택배, 04=우편(소포/등기), 05=직접전달(화물배달), 06=퀵서비스, 99=배송없음) |
| &nbsp;&nbsp;&nbsp;&nbsp;`freeGiftNo` | 사은품번호 | integer | Y |  |  |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`freeGiftQty` | 사은품수량 | integer | Y |  |  |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`gblDlvYn` | 전세계배송여부 | string | Y | 1 | N |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`giftCd` |  | string | Y |  |  | - api 사용유저는 해당사항 없음 |
| &nbsp;&nbsp;&nbsp;&nbsp;`invcNo` | 송장번호 | string | Y | 50 |  | 2015/03/20(금)부터 신규제공 |
| &nbsp;&nbsp;&nbsp;&nbsp;`lstDlvCst` | 배송비 | integer | Y | 12 | 2800 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`ordStlEndDt` | 결제완료일시 | string | Y |  | 2015-01-13 16:52:24 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`plcodrCnfDt` | 발주확인일시 | string | Y |  | 2015-01-13 17:02:57 |  |
| &nbsp;&nbsp;&nbsp;&nbsp;`sndEndDt` | 발송처리일 | string | Y |  | 2015-01-13 17:05:22 | 2015/03/20(금)부터 신규제공 |
| &nbsp;&nbsp;&nbsp;&nbsp;`typeAdd` | 주소 유형 | enum | Y | 5 | 01 | (코드값: 01=지번명, 02=도로명) |
...
|---|---|---|---|---|---|---|
| `ns2:orders` | ns2:orders | object | Y |  |  |  |
| &nbsp;&nbsp;`ns2:result_code` | 결과코드 | enum | Y |  | 0 | (코드값: 0=조회된 결과가 없습니다.<br>설명 - 조회된 결과값이 없을 경우입니다. 에러가 아닙니다., -1=ex)주문/클레임 조회 오류 MSG : OpenAPI Key 에 해당하는 유저가 없습니다.<br>비지니스 Error. 예외적으로 발생되는 모든 에러. 메시지는 일정하지 않습니다., -3102=start_dt의 조회 기간의 포멧(&#39;YYYYMMDDHH24:MI&#39;)이 올바르지 않습니다.<br>설명 - start_dt 조회기간의 데이터 포멧 문제 예)201005041400, -3103=end_dt의 조회 기간의 포멧(&#39;YYYYMMDDHH24:MI&#39;)이 올바르지 않습니다.<br>설명 - end_dt 조회기간의 데이터 포멧 문제 예)201005041400, -3104=최대 조회기간은 일주일 입니다.<br>설명 - end_dt - start_dt 의 값이 8일 이상일 경우 입니다., -1000=서버 점검중입니다.<br>설명 - 매주 금요일 새벽은 정기점검일입니다. 서버 차단이 있을수 있습니다.) |
