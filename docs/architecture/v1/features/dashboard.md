...
2. **마켓별 주문 현황** — "어느 마켓에 신규 주문이 몇 건 들어왔고, 동기화는 정상인가." (v1 정식 4 마켓 + 11번가 placeholder)
...
| 마켓별 주문 현황 | 4 마켓 (네이버 / 쿠팡 / G마켓 / 옥션) 카드 + 11번가 `comingSoon` placeholder. 신규 주문 카운트 + 오늘 총합 + 동기화 상태 + 마지막 sync 시각 | 마켓별 매출, 기간 비교, 채널 광고 KPI |
...
TwoColumnSection
  ...
  │     ├ Grid 2×2: MarketOrderItemCard × 4 (네이버 / 쿠팡 / G마켓 / 옥션)
  │     │   item:
  │     │     ├ 좌측 컬러바 (브랜드 색)
  │     │     ├ 색상 도트 + 마켓명
  ...
  │     └ comingSoon row: 11번가 placeholder (dimmed)
  │
...
- Loading: 4 카드 grid skeleton + 11번가 row skeleton
- Error: `role="alert"` + `<ErrorMessage details>` (raw 메시지 접힘)
- Data/Empty: 2×2 마켓 카드 grid + `comingSoon` row (현재 `['11st']`)
...
| MarketOrdersSummaryCard | 4 카드 + 1 row skeleton | 4 마켓 카드 + 11번가 placeholder | 컨테이너 단문 에러 (role=alert) — 다른 위젯에 영향 없음. raw 메시지는 접힘 | `markets[].newOrdersCount + todayTotalCount === 0` 전체일 때: 신규 없음 회색 표시 + (마켓 0건 시) 마켓 연결 안내 |
...
6. MarketOrderItemCard 1 → 4 (네이버 / 쿠팡 / G마켓 / 옥션. 각 카드 = link, Enter 로 /orders/list 또는 /markets 진입)
7. (11번가 placeholder 는 link 아님, Tab skip — aria-disabled 가 아니라 정적 row)
8. "마켓 관리 →" 링크 (MarketConnectionStatusCard)
...
- MarketOrderItemCard link: `aria-label="네이버 주문 12건, 목록으로 이동"` / 오류 시 `"네이버 연결 오류, 재인증 페이지로 이동"`
- 11번가 placeholder row: `aria-label="11번가 오픈 준비중"` (link 아님)
...
| QA-DSH-007 | 셀러 A 의 네이버 신규 주문 12건, 쿠팡 0건, G마켓 sync 오류, 옥션 정상 0건. | `/dashboard` 진입 | 4 카드 시각 검증: 네이버 = 12 신규 강조, 쿠팡 = 0 신규 dimmed, G마켓 = border-danger + AlertCircle 배지, 옥션 = 0 신규 dimmed + 정상 배지. 11번가 placeholder row dimmed. | Playwright | P0 |
...
| QA-DSH-011 | 키보드만 사용. Tab 으로 순회. | dashboard 진입 후 Tab 반복 | "본문으로 건너뛰기" → 새로고침 → CTA → KPI 카드 → "주문 전체 보기" → 마켓 카드 1~4 → "마켓 관리" 순. focus ring 명확. Enter 로 모든 인터랙티브 요소 활성화. 11번가 placeholder skip. | Playwright + axe | P0 |
...
- [ ] axe-core 위반 0건 (CI).
- [ ] 키보드만으로 모든 인터랙티브 요소 도달 (11번가 placeholder Tab skip).
...
- [ ] 11번가 placeholder 가 `comingSoon` 배열에서 자동 구성 (하드코딩 row 금지).
...
| OPEN-DSH-008 | `comingSoon` 배열을 코드 상수에서 DB 설정 테이블 (`market_feature_flags`) 로 이관할지. 11번가 진입 시점에 코드 수정 vs 운영 토글. | 11번가 어댑터 v2 진입 직전 | 운영 |
