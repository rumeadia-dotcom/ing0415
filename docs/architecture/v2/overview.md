# v2 — 주문·배송 자동화 아키텍처 개요

> **버전**: v2.0-draft | **작성일**: 2026-05-21
> **범위**: 4 마켓(네이버/쿠팡/G마켓/옥션) 신규 주문 폴링 → 로젠택배 집하 자동 등록 → 운송장 1클릭 출력 → 마켓 송장 1클릭 일괄 제출.
> **전제**: v1 (상품 등록 자동화) 운영 중. 로젠택배 B2B 계약 완료.

---

## 1. 목적

PRD-v2-shipping.md / user_flow-v2-shipping.md 가 ground truth. 본 문서는 그 두 문서를 묶어 **아키텍처 결정과 디렉토리 매핑**을 단일 진입점으로 제공한다.
...
| 마켓 어댑터 | 5 메서드 (authenticate / refreshToken / fetchCategoryTree / transformProduct / createProduct) | **+2 메서드**: `fetchOrders` / `submitTracking` (cross-cutting/market-adapter-v2.md) |
...
**기존 도메인 변경 없음.** v1 의 `MarketAccount` (OAuth/HMAC/ESM 토큰) 를 그대로 재사용 — v2 가 v1 위에 얹히는 구조. 토큰 복호화 RPC (`market_credentials.decrypt_token`) 동일 경로.
...
- **OQ-V2-02 해결 후**: 마켓별 웹훅 진입을 추가 Edge Function 으로 받고, pg_cron 은 fallback / 손실 보정 용으로 유지.
...
cross-cutting/logen-adapter.md 가 로젠 API 4 메서드 명세, cross-cutting/market-adapter-v2.md 가 마켓별 fetchOrders / submitTracking 매트릭스를 정의한다.
...
cross-cutting/market-adapter-v2.md 에 매트릭스 정의.
...
PR1 (foundation)  ─┬─→ PR2 (schemas + migrations)
                   ├─→ PR4 (market-adapter v2: fetchOrders / submitTracking)
...
본 PR (foundation) 은 다른 9 개 PR 의 **충돌 표면을 선점**하는 역할로, 라우터·사이드바·i18n·디렉토리 트리·설계문서만 추가하고 실제 비즈니스 로직·DB 마이그레이션·Edge Function 코드는 일절 손대지 않는다.

---

## 6. MVP 범위 게이트

CLAUDE.md "MVP 범위 (v1)" 의 v1 항목은 변경 없음. v2 는 별도 마일스톤이며, 본 문서가 정의하는 도메인 모두 **v1 출시 + 셀러 운영 데이터 일정 기간 확보 이후** 단계적으로 활성화한다. 본 파운데이션 PR 은 라우트/메뉴를 노출하되 placeholder 표시(`v2 작업 진행 중`)로 사용자가 잘못 진입했을 때 혼란이 없도록 한다.
...
| # | 질문 | 영향 |
|---|---|---|
| OQ-V2-02 | 마켓 주문 웹훅 지원 여부 | 폴링 주기 / Realtime 정밀도 |
| OQ-V2-03 | 네이버 주문 API 와 상품등록 앱 동일 여부 | 인증 분리 필요 시 `MarketAccount` 분기 |
| OQ-V2-04 | `registerOrderData` 의 `fareTy` / `dlvFare` 운영값 | s9 n60 발송인 정보 폼 기본값 |
| OQ-V2-05 | "출력 후 자동 제출" 기본 ON/OFF | s9 n58 설정 default |
