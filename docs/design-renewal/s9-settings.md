...
| `/settings/shipping/esm-profiles` | `/settings/shipping` → "G마켓·옥션 배송 프로필" 카드의 [배송 프로필 관리] | yes |
...
| n61 (G마켓·옥션 배송 프로필) | §1.2 / §1.3 (ESM 등록 선행값) — esm.md §3/§5 | ESM 4단계(주소록→출하지→묶음배송→발송정책) 생성·재사용. s3 등록은 select 만 (PR-3) |
...
| `/settings/shipping/esm-profiles` | `apps/web/src/features/settings/shipping/pages/SettingsShippingEsmProfilesPage.tsx` | G마켓·옥션 배송 프로필 (n61) | 설정 |
...
└─ /settings/shipping     # SettingsShippingPage — 배송 설정 허브 (n58)
    ...
    │
    └─ 카드 5: G마켓·옥션 배송 프로필 (ESM)
        ├─ Badge: 등록된 프로필 N개 (count > 0)
        └─ [배송 프로필 관리] → /settings/shipping/esm-profiles (n61)
...
### 4.7 SettingsShippingEsmProfilesPage — `/settings/shipping/esm-profiles` (n61)
