...
| ~~n61 (G마켓·옥션 배송 프로필)~~ — 제거됨 (PR-E3) | §4.7 (제거 기록) | 조회형 전환으로 제거. ESM 배송 선행값은 ESM Plus 등록 → s3 에서 GET 조회·select (esm.md 전환 결정 절) |
...
└─ /settings/shipping     # SettingsShippingPage — 배송 설정 허브 (n58)
    ...
    └─ 카드 4: 기본 택배사
        ...
        # 카드 5 (G마켓·옥션 배송 프로필, ESM 생성형)은 조회형 전환으로 제거됨 (2026-05-30, PR-E3).
        #   ESM 배송 선행값은 ESM Plus 에서 등록 → 상품등록 3단계에서 GET 조회·select (esm.md 전환 결정 절).
...
### 4.7 ~~SettingsShippingEsmProfilesPage — `/settings/shipping/esm-profiles` (n61)~~ — 제거됨 (2026-05-30, PR-E3)

> **조회형 전환(`esm.md` 전환 결정 절)으로 본 화면·라우트·n61·카드 5·생성 훅/api/Edge·`esm_shipping_profiles` 테이블이 모두 제거됐다(PR-E3/E4).** ESM 배송 선행값을 우리 앱이 생성하지 않고, 셀러가 ESM Plus 에서 만든 출하지·발송정책을 상품등록 3단계(s3 §MarketOptionsCard)에서 GET 조회·select 한다(조회형 — `useEsmShippingOptions` + Edge `esm-shipping-list`). 디자이너 인계 시 본 화면은 더 이상 그리지 않는다. 생성형 구현 상세는 git 이력(이 절의 이전 리비전)을 참조.
