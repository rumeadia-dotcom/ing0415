...
| ~~`/settings/shipping/esm-profiles`~~ ⚠️ deprecate 예정 (조회형 전환, PR-E3) | `/settings/shipping` → "G마켓·옥션 배송 프로필" 카드 | yes |
...
| ~~n61 (G마켓·옥션 배송 프로필)~~ ⚠️ deprecate 예정 | §4.7 (생성형 기록) | 조회형 전환으로 제거(PR-E3). ESM 배송 선행값은 ESM Plus 등록 → s3 에서 GET 조회·select (esm.md 전환 결정 절) |
...
| ~~`/settings/shipping/esm-profiles`~~ ⚠️ deprecate 예정 (PR-E3) | `SettingsShippingEsmProfilesPage.tsx` | G마켓·옥션 배송 프로필 (n61) — 조회형 전환으로 제거 | 설정 |
...
└─ /settings/shipping     # SettingsShippingPage — 배송 설정 허브 (n58)
    ...
    │
    └─ ⚠️ 카드 5: G마켓·옥션 배송 프로필 (ESM) — DEPRECATE 예정 (조회형 전환, PR-E3 제거)
        ├─ (생성형 시절: 등록된 프로필 N개 Badge + [배송 프로필 관리] → /settings/shipping/esm-profiles)
        └─ 조회형 전환 후: 이 카드/페이지 제거. ESM 배송 선행값은 ESM Plus 에서 등록 → 상품등록 3단계에서 GET 조회·select (esm.md 전환 결정 절)
...
### 4.7 SettingsShippingEsmProfilesPage — `/settings/shipping/esm-profiles` (n61) ⚠️ DEPRECATE 예정

> **조회형 전환(2026-05-30, `esm.md` 전환 결정 절)으로 본 화면·라우트·n61 은 제거된다(PR-E3).** ESM 배송 선행값을 우리 앱이 생성하지 않고, 셀러가 ESM Plus 에서 만든 출하지·발송정책을 상품등록 3단계에서 GET 조회·select 한다. 아래는 생성형 구현 기록(deprecate 대상)으로 보존.

- **파일**: `apps/web/src/features/settings/shipping/pages/SettingsShippingEsmProfilesPage.tsx`
- **목적**: G마켓·옥션(ESM) 상품 등록 선행값(출하지·발송정책)을 마켓 계정별로 **1회 생성·재사용**하는 "배송 프로필" 관리. 상품 등록(s3 3단계)은 이 프로필을 select 만 — 본 화면이 유일한 생성 진입점 (esm.md §1.3: 등록 폼 안에서 생성 API 호출 금지, 실패 시 고아 정책 방지).
- **진입 경로**: `/settings/shipping` 의 카드 5 [배송 프로필 관리]. ESM 계정 미연결 시 `/markets/connect` 유도.
- **user_flow 노드**: n61
- **PRD 근거**: §1.2 / §1.3 (마켓별 등록 선행값) — esm.md §3 / §4.5 / §5 / §7(PR-3)
- **기능**:
  - 목록: 셀러의 ESM 배송 프로필 (`useEsmShippingProfiles`) — `profileLabel` / 마켓(Badge G/A) / 발송유형 / 배송비 / `dispatchPolicyNo` / 상태 Badge.
  - 생성: [새 배송 프로필] → Dialog (RHF + `EsmShippingProfileCreateInputSchema`). 입력 = 마켓 계정 select(연결된 gmarket/auction active 만) / profileLabel / dispatchType(A~F) / shippingFee / feeType(1·2) / 주소(우편번호·기본·상세·담당자·연락처). site 는 선택 계정의 marketId 에서 파생.
  - 제출 → `useCreateEsmShippingProfile` mutation → Edge `esm-shipping-profile` 가 ESM 4단계 생성(주소록→출하지→묶음배송→발송정책) + INSERT → 성공 시 toast + 목록 invalidate.
  - **PII 경계**: 주소·담당자·연락처는 Edge 호출 바디에만 — DB 엔 번호만 저장(esm.md §3). UI 안내 문구로 명시.
- **주요 컴포넌트**: `PageHeader`, `SettingsNav`, `Card`, `Badge`, `Dialog`, `Input`, `Label`, raw `<select>` (shadcn Select 미사용 — sender 페이지와 동일 컨벤션), `Tooltip` (생성 버튼 blockingReasons), `ErrorMessage`, `Skeleton`, `Button`
- **데이터 의존**: `useMarketAccounts()` (ESM 계정 게이팅) / `useEsmShippingProfiles()` (목록) / `useCreateEsmShippingProfile()` (생성)
- **상태 처리 (4상태 + 게이팅)**:
  - `loading`: 프로필 로딩 Skeleton
  - `error`: ErrorMessage (`errors.fetch`)
  - `data`: 프로필 행 리스트
  - `empty`: 프로필 0 + ESM 계정 있음 → 빈 안내 + [새 배송 프로필] 활성
  - `no-ESM-account`: ESM 계정 미연결 → 생성 불가 안내 + 버튼 disabled + tooltip(`blocking.noAccount`) + [마켓 계정 연결] CTA
- **실행 버튼 규약**: [새 배송 프로필] = 실행류(서버 변경). ESM 계정 없으면 disabled + blockingReasons tooltip.
