...
| 카테고리 검색 (추천 Phase 1) | `CategorySearchBox` 에 카테고리명 타이핑(≥2자, 250ms 디바운스) → 전역 인덱스 leaf 부분일치 결과를 listbox 로. 클릭/Enter 로 leaf 즉시 확정(cascader 안 거침). ESM 인덱스 빌드 중이면 "색인 준비 중" + 4s 폴링. 네이버 미노출. (Edge `markets-category-search`, `category-sync.md §10`) |
| 최근 카테고리 칩 (추천 Phase 1) | `RecentCategoryChips` — 셀러 마켓별 최근 사용 6개 칩(표시=leaf, hover=전체경로). 클릭 시 즉시 leaf 확정. 0개면 미노출. (RPC `get_recent_categories`) |
| 카테고리 단계 선택 (lazy cascading) | 마켓별 카테고리는 `CategoryCascader` 단계별 select — `useMarketCategoryChildren(marketId, marketAccountId, parentId)` (Edge `markets-category-children` 경유, 부모코드 직계 자식만 lazy 조회). 브라우저 직접 fetch 금지(CORS) — Edge→Gateway→마켓. 네이버는 미지원(`category_not_supported`) → 안내 카드. **검색·최근 칩이 막혔을 때의 graceful 폴백** |
| 카테고리 leaf 확정 | leaf 노드 선택(검색/최근/cascader 공통) 시 `upsertMapping(next)` — 같은 marketId 의 이전 매핑 교체. cascader 비-leaf 선택은 다음 단계 자식 조회(미확정) |
| 마켓별 override (이름·가격·옵션) | 같은 카드에서 입력 → mapping 업데이트 |
| "다음: 미리보기" | `Step3Schema.safeParse` 통과 시 `/register/preview` navigate |

**주요 컴포넌트**:
- 자체: `MarketSelectGrid` (v1 5마켓 카드 그리드, 비활성 계정 마켓은 disabled + 사유 라벨), `CategorySearchBox` (카테고리명 검색 combobox+listbox — 5상태 idle/loading/building/empty/ready, 키보드 ↑↓/Enter, 추천 Phase 1, 네이버 미노출), `RecentCategoryChips` (최근 사용 6개 칩, 0개 null), `CategoryCascader` (부모코드 직계 자식만 조회하는 단계별 select — 대→중→소 lazy, leaf 확정 = marketCategoryCode, 4상태 + 네이버 미지원 안내. Edge `markets-category-children` 경유. 검색·최근의 graceful 폴백), `MarketOptionsCard` (검색박스 + 최근칩 + CategoryCascader + 어댑터 메타 기반 동적 등록필드. ESM=출하지·발송정책 조회형 select + 상품정보고시 / 11번가=출고지·반품지 조회형 select + 상품정보고시), `OfficialNoticeField` (상품정보고시 상품군 select + 군별 항목 동적 폼 — ESM PR-5 / 11번가 PR-4 공용, 마켓별 상품군 마스터는 `config` prop 주입)
...
- `useCategorySearch(marketId, marketAccountId, query)` — 카테고리명 부분일치 검색 (Edge `markets-category-search`, 250ms 디바운스, q≥2 & !naver, `building` 시 4s 폴링). 추천 Phase 1.
- `useRecentCategories(marketId)` — 마켓별 최근 사용 카테고리 6개 (RPC `get_recent_categories`, staleTime 10m, !naver).
