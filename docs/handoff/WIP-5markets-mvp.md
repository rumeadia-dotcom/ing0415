...
**develop HEAD**: `c6a419f` (+ 미커밋: WYSIWYG·발송지·배송정책·mock fix) — 765 passed / 26 todo / 0 failed
**main HEAD**: `19a2aa9` — release/v0.5 운영 배포 완료 (2026-05-22T03:39)
**활성 PR (develop base)**: #76 주문·배송 후속 정합 (release/v0.6 포함 예정) / dependabot 7건
...
| **v0.6 진행 중** | WYSIWYG 에디터 + 발송지 주소 API + 배송 정책 화면 + mock fix | **미커밋 → release/v0.6 예정** |

## v0.6 신규 기능 — 상세 (2026-05-22)

### WYSIWYG 에디터 (PRD §3.6.1 / §3.6.2)
- Tiptap (StarterKit + Link + Image + Placeholder) + DOMPurify sanitize
- `RichTextEditor` 공통 컴포넌트 (`apps/web/src/components/ui/rich-text-editor.tsx`)
- `sanitizeHtml()` — XSS 차단 허용 태그 화이트리스트 (`apps/web/src/lib/sanitize-html.ts`)
- `StepInfoPage` 상품 설명 textarea → RichTextEditor 교체 (Controller)

### 발송지 주소 Daum Postcode API (PRD §2.2.1 인접)
- `daum-postcode.ts` — SDK 동적 로드 (script 태그, API key 불필요)
- `AddressSearchInput` 컴포넌트 — 도로명 주소 read-only + 상세 주소 수기 입력 + 검색 버튼
- `SettingsShippingSenderPage` address 필드 Controller 교체
- mock `daum-postcode` 테스트 모킹 추가 → sender 테스트 4건 통과

### 배송 정책 등록/수정 화면 (PRD §1.1.4)
- `/settings/policies` 신규 (라우트 + 페이지 + 4상태 + 생성/수정/삭제/기본값 토글 Dialog)
- `ShippingPolicyFormSchema` zod 단일 소스 (`apps/web/src/lib/schemas/shipping-policy.ts`)
- `useShippingPolicies` — `useUpdateShippingPolicy` / `useDeleteShippingPolicy` hook 추가
- 기본값 클리어: 클라이언트 2-step (다른 row → is_default=false → 대상 → true)
- DB: `shipping_policies` 테이블은 `20260519000005_products.sql` 에 기존 존재 (마이그레이션 불필요)
- `SettingsNav` 에 배송 정책 항목 추가

### mock insert id 버그 수정
- mock `insert` 시 `id` 없으면 `crypto.randomUUID()` 자동 생성
- 원인: id 없는 row → option value undefined → select 값 '' → zod uuid() 검증 실패
- 부수 수정: 상품 draft insert id 없어 `currentProductId` 비교 실패 → 항상 duplicate 오판

## 운영 현황

- **운영 배포 URL**: `https://rumeadia-dotcom.github.io/ing0415/`
- **최근 deploy**: release/v0.5 → `completed / success` (2026-05-22T03:39)
...
### 🟡 주문·배송 후속 정합 — **PR #76 release/v0.6 포함 예정**
...
### 우선 순위
1. **release/v0.6 완료** — PR #76 머지 → release/v0.6 생성 → CI 통과 → main 머지 → deploy.yml 성공
2. **설계문서 sweep** — `design-renewal/s3-register.md` / `s9-settings.md` + `features/registration.md` 갱신
3. **phase 2: dev DB seed** — `apps/api/supabase/seed.sql` 작성 → `pnpm dev:db` 투어
