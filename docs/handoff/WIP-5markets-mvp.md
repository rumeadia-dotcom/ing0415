...
## Phase 2 진행 중 (2026-05-20 — B-1·B-2·B-3 완료, B-4 Phase 0 완료)

```
B-1 환경 셋업 ✅ → B-2 s1 인증 ✅ → B-3 s5 마켓계정 ✅ → B-4 s3 상품등록 🔶 진행 중
   ↓                  ↓                 ↓                    ↓
Supabase 2 프로젝트   4페이지 본구현    4페이지 본구현       5단계 위저드
GitHub Secrets        AuthProvider     useQuery + Realtime   RHF + zod + zustand store
Branch protection     RequireAuth      4상태 + partial       이미지 파이프라인
CI 6/6 ✓ + Pages      105 단위테스트   33 단위 (합산 125)    Phase 0 ✅ (api/hooks/store/utils)
                                                            Phase 1~6 ⬜ (Step 1 ⬅ 다음)
누적 테스트: 156 passed / 0 failed / 26 todo
```

## 현재 develop 브랜치 (HEAD = `a58f424`, 2026-05-20 10:04)

**Working tree 상태**: B-4 Phase 0 작업물 untracked (commit 직전 단계). 변경:
- `package.json` / `pnpm-lock.yaml` — zustand `^5.0.13` 추가
- `apps/web/src/features/registration/` 신설: `api/` 3종 (registration / image / category) + `hooks/` 8종 (useRegistrationStart/Validate/Cancel/Retry/Job + useImageUpload + useDuplicateProductCheck + useMarketCategoryTree) + `store/useRegisterFormStore.ts` (zustand 폼 store) + `utils/registration-error-messages.ts` + `__tests__/` 2종
- 본 WIP 문서
...
a58f424 feat(markets): B-3 s5 마켓계정 본구현 — 4페이지 + Realtime + 4분기 인증  ⬅ HEAD
```

검증 상태 (2026-05-20 10:04, working tree 포함):
- `pnpm typecheck` ✅
- `pnpm test` ✅ **156 passed / 0 failed / 26 todo** (B-4 Phase 0 단위 테스트 19건 신규: registration-error-messages 11 + useRegisterFormStore 8)
- `pnpm lint` ✅ 0 error (마지막 측정 시점 — Phase 0 코드는 typecheck 통과로만 검증, lint 는 Phase 1 진입 직전 재실행 예정)
- `pnpm build:debug` — Phase 0 단계 미실행 (Phase 1 진입 직전 1회)
- CI on `a58f424`: ✅ 6/6 통과 (1m35s) — 다음 push 는 Phase 1 완료 후

**다음 작업**: B-4 Phase 1 (Step 1 정보 입력 RHF + zod + 디바운스 중복 확인). Plan: `~/.claude/plans/2026-05-20-b4-registration-wizard.md`.
...
2. **현재 상태 확인** — `pnpm install` (zustand 동기화) → `pnpm test` 156 passed 확인 → Phase 0 산출물 (`apps/web/src/features/registration/`) untracked 인 채로 Phase 1 진입 가능.

## 다음 작업 — B-4 Phase 1 (Step 1 정보 입력, 1~1.5일) ⭐

Plan: `~/.claude/plans/2026-05-20-b4-registration-wizard.md` Phase 1 섹션 참조.

핵심 산출물:
- [ ] **RegisterStep1Page** (`apps/web/src/features/registration/pages/RegisterStep1Page.tsx`) — RHF + zodResolver(`Step1FormSchema`).
- [ ] **Step1FormSchema** (`apps/web/src/lib/schemas/registration.ts` 에 추가) — productName / price / shippingFee / detailHtml / brand / manufacturer 등.
- [ ] **useDuplicateProductCheck** hook (Phase 0 작성됨) 를 productName onChange + 디바운스(300ms) 로 연결.
- [ ] **useRegisterFormStore** (Phase 0 zustand store) 에 Step 1 setStep1 액션 호출 + sessionStorage persist 검증.
- [ ] 다음 버튼: store 저장 + `navigate('/register/images')` (Step 2 placeholder 라우트 추가).
- [ ] 단위 테스트: form validation (필수 필드 / 가격 음수 / 디바운스 동작).
- [ ] HTML 프로토타입 / 설계문서 sync 는 **Phase 6 commit 직전 일괄** (B-4 wizard commit 전략 참조).

후속 (B-4 외 잔여):
- [ ] s1 LoginPage / SignupPage 통합 테스트 (RTL) — B-2 후속, 분량 작음 (반나절). B-4 종료 후 처리 또는 v2.
- [ ] auth-event-log Edge Function 호출 통합 (login_success / login_failure / password_reset_*) — v2 백로그 유지 가능.
...
→ http://localhost:5173/register 진입 (현재 placeholder). Phase 1 작업은 `apps/web/src/features/registration/pages/` 에 RegisterStep1Page 신설부터.

## B-4 wizard commit 전략 (2026-05-20 확정)

Phase 0~5 를 phase 별 commit 하지 않고 **wizard 전체 완성 후 단일 PR 로 squash 머지** 한다. 이유: zustand store / api / hooks 가 Phase 5 까지 함께 진화하면서 중간 commit 의 의미가 낮음. **Phase 6 sync sweep + `/git-commit` 스킬 호출이 유일한 commit boundary**. 그 전까지 working tree untracked 누적은 의도된 상태.
