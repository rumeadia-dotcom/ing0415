...
## Phase 2 진행 중 (2026-05-20 — B-1·B-2·B-3·B-4 완료, B-5 다음)

```
B-1 환경 셋업 ✅ → B-2 s1 인증 ✅ → B-3 s5 마켓계정 ✅ → B-4 s3 상품등록 ✅ → B-5 s2 대시보드 + s6 등록이력 ⬜
   ↓                  ↓                 ↓                    ↓                    ↓
Supabase 2 프로젝트   4페이지 본구현    4페이지 본구현       5단계 위저드본구현    Plan 작성됨 (5~7일 추정)
GitHub Secrets        AuthProvider     useQuery + Realtime   RHF + zod + zustand   /dashboard /history* 본구현
Branch protection     RequireAuth      4상태 + partial       이미지 파이프라인     RPC/view 이미 적용됨 (DB 작업 0)
CI 6/6 ✓ + Pages      105 단위테스트   33 단위 (합산 125)    Phase 0~6 ✅          ⬅ 다음 세션 진입
누적 테스트: 173 passed / 0 failed / 26 todo (B-4 통합 후)
```

추가 작업물 (2026-05-20 14:xx, working tree untracked):
- **설정 페이지 + 로그아웃** — `/settings` 라우트 신설 + 사이드바 "설정" 활성. `apps/web/src/features/settings/pages/SettingsPage.tsx` + locale + a11y ROUTES + .gitignore (`/supabase/` stray 차단).

## 현재 develop 브랜치 (HEAD = `0014ae9`, 2026-05-20 11:xx)

**Working tree 상태**: 설정/로그아웃 + WIP 갱신 + Plan 작성 untracked (commit 직전 단계).
...
0014ae9 feat: B-4 s3 상품등록 5단계 위저드 본구현  ⬅ HEAD
```

검증 상태 (2026-05-20 11:56, working tree 포함):
- `pnpm typecheck` ✅
- `pnpm test` ✅ **173 passed / 0 failed / 26 todo** (B-4 통합 후 신규: registration step + StepInfo/StepImages/StepMarkets/preview/result 단위 19건)
- `pnpm lint` — 다음 commit 직전 1회 (작업 단위 종료 시점만 실행, 로컬 hook 룰)
- `pnpm build:debug` — 다음 commit 직전 1회
- CI on `0014ae9`: ✅ 6/6 통과 — 다음 push 는 설정/로그아웃 + B-5 Phase 4 완료 후

**다음 작업**: B-5 Phase 0 (api + schemas + hooks 골격). Plan: `~/.claude/plans/2026-05-20-b5-dashboard-history.md`.
...
2. **현재 상태 확인** — `pnpm test` 173 passed 확인 → working tree untracked (설정/로그아웃 + WIP/Plan) 검토 후 B-5 Phase 0 진입.

## 다음 작업 — B-5 Phase 0 (api + schemas + hooks 골격, 1일) ⭐

Plan: `~/.claude/plans/2026-05-20-b5-dashboard-history.md` Phase 0 섹션 참조.

핵심 산출물:
- [ ] **dashboard-api.ts** — `rpc_get_dashboard_summary().maybeSingle()` + `rpc_get_recent_jobs(20)` + zod parse.
- [ ] **history-api.ts** — `list_registration_jobs(...)` + `get_registration_job(p_job_id)` invoke + zod parse.
- [ ] **schemas 확장** — `dashboard-summary.ts` (RecentJob + MarketHealth) + 신규 `history.ts` (HistoryFilter URL ↔ state + ListItem + Detail).
- [ ] **hooks 5종 골격** — useDashboardSummary / useRecentJobs / useHistoryList / useHistoryDetail / useHistoryFilterState.
- [ ] 단위 테스트 +5건 — schemas pass/fail + filter URL round-trip.

후속 Phase:
- **Phase 1** Dashboard 본구현 (1~1.5일)
- **Phase 2** History 목록 + 필터 (1.5~2일)
- **Phase 3** History 상세 + 재시도/제외 액션 (1~1.5일)
- **Phase 4** sync sweep + 검증 + commit (0.5~1일)

후속 (B-5 외 잔여):
- [ ] 설정/로그아웃 working tree commit — B-5 Phase 0 시작 전 별도 commit (v2 옵션) **또는** B-5 Phase 4 commit 에 합쳐 squash.
...
→ http://localhost:5173/dashboard 진입 (현재 placeholder). Phase 0 작업은 `apps/web/src/features/dashboard/api/dashboard-api.ts` 신설부터.

## B-5 commit 전략 (2026-05-20 확정)

B-4 와 동일 — Phase 0~3 누적 후 Phase 4 sync sweep + 단일 commit + squash 머지. 중간 commit boundary 없음. 설정/로그아웃 작업물은 별도 commit 으로 먼저 분리할지 B-5 squash 에 합칠지 Phase 4 시점에 판단.
