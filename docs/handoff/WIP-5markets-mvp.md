...
**develop HEAD**: `5e9a4b5` — chore(deps): minor-and-patch 4건 bump (#282)
**main HEAD**: `3e4db98` — release: v0.16 (#280) · **Deploy (real) success (2026-05-30 09:36)** — 운영 배포 완료
**테스트**: 1352 passed / 1 skipped / 31 todo (118 files)
**갱신일**: 2026-05-30
**v0.16 범위**: 11번가 #270~#279(PR-1~6) · ESM 조회형 전환 #270·274·275·277(E1~E5) · 백머지 #285 · CI 빠른레인 #286 · deps #282
...
- **C2 게이트웨이** — `api.11st.co.kr`+`sa2.esmplus.com` allowlist 반영, healthz 200. ⚠️ **PR-6 maskUrl(송장 마스킹) 운영 박스 재배포 대기** (아래 운영 액션 1).
- **C1 dev DROP 완료** — `esm_shipping_profiles` dev 테이블 제거 확인. 마이그 **이력 정합**만 잔여(운영 액션 2).
...
| **C7** | **잠재 Deno 타입 에러** | `deno check` 발견 3건(audit sellerId / eleven-st expiresAt / validate enum). vitest 미검출. enum 누락(③)은 런타임 영향 가능 → **우선 점검 중.** |
...
- **dev Supabase** (`eqoyw`, 뭄바이): `esm_shipping_profiles` DROP 완료. 마이그 이력 정합 미완(운영 액션 2 / C1).
- **real Supabase** (`lfrny`, 서울): ESM/11번가 신규 마이그 미적용 (real DB 변경 없음 — 어댑터·Edge 코드만).
- **Lightsail 게이트웨이**: allowlist 반영됨. **maskUrl(PR-6) 재배포 대기**(운영 액션 1).
...
### 우선 순위
1. **운영 액션 2건** — Lightsail 재배포(maskUrl) + dev 마이그 이력 정합(C1). (사용자 수동 SSH/DB)
2. **C7 Deno 타입 에러 ③(validate enum) 우선 점검** — 런타임 영향 가능. (코드로 진입 가능)
3. **C3 real 실호출 검증** — 셀러 키 발급 + IP 화이트리스트 후 5마켓 1회 실호출. **다음 release 핵심 게이트** (현재 키 미발급 보류).
4. **NEW-1/NEW-2 plumbing** — 11번가 dlvNo(발송) + KC certRequiredYn(상품등록) 워커 경로.

> v0.16 배포 완료. 다음 release 는 C3 실호출 검증 후 사용자 결정.
