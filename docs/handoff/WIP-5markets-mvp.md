...
## Phase 2 진행 중 (2026-05-20 — B-1, B-2, B-3 본구현 완료)

```
B-1 환경 셋업 ✅ → B-2 s1 인증 ✅ → B-3 s5 마켓계정 ✅ → B-4 s3 상품등록 ⬅ 다음
   ↓                  ↓                 ↓                    ↓
Supabase 2 프로젝트   4페이지 본구현    4페이지 본구현       5단계 위저드
GitHub Secrets        AuthProvider     useQuery + Realtime   RHF + zod
Branch protection     RequireAuth      4상태 + partial       이미지 파이프라인
CI 6/6 ✓ + Pages      105 단위테스트   33 단위 (합산 125)    Realtime 진행률
```
...
- [x] **MarketsListPage placeholder → 본구현** (`apps/web/src/features/markets/pages/MarketsListPage.tsx`)
...
- [x] **MarketsConnectProviderPage 4분기 본 동작**:
  ...
  - 11번가 → disabled 안내
- [x] **OAuthCallbackPage**: code + state 검증 + `markets-oauth-callback` invoke + 성공/실패 화면 (markets.md §7.4 와이어 준수)
- [x] **연결 해제 / 재인증 / verify 동작** — MarketAccountActions (`useDisconnectMarket` / `useVerifyMarket` + Dialog confirm)
- [x] **에러 매핑** — `formatMarketError` 13종 + correlationId 노출 (markets.md §10)
- [x] **단위 테스트 33건 추가** (market-error-messages 17 / useConnectMarket 2 / MarketsListPage 5 / MarketsConnectProviderPage 4 / OAuthCallbackPage 4 + 기존 통합 92 → 합산 125 통과)
