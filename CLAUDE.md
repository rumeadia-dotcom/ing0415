...
**모든 설계 결정은 사용자(셀러) 편의를 최우선 기준으로 한다.** 기술적 단순함·구현 비용보다 "셀러가 더 적은 수고로 일을 끝내는가"가 우선한다. 아래 규칙들과 충돌하면 이 원칙이 방향을 정하고, 나머지는 그 방향을 안전하게 구현하는 수단으로 본다.

- 반복·수고가 큰 흐름(깊은 cascading, 재입력, 동일 작업 반복)은 추천·기억·자동완성·기본값으로 줄인다.
- 기능을 "동작하게"가 아니라 "쉽게"까지 설계한다 — 행복경로뿐 아니라 막혔을 때의 안내·복구 경로까지 사용자 관점으로 포함.
- 구현이 비싸다는 이유로 사용자 편의를 깎지 않는다. 비용이 크면 단계를 쪼개되(예: Phase 0/1) 편의 목표 자체는 유지한다.
- **근거**: 2026-05-31 사용자 지시 — "모든 설계는 사용자 편의 중심으로".
