/**
 * Sentry environment 분리 회귀 테스트 — dev ↔ real 데이터 혼입 차단.
 *
 * 출처 / 근거:
 *   - 작업 카드: D-D (WIP-5markets-mvp.md Phase 4)
 *   - 마스터: docs/architecture/v1/security.md §6.3 (environment = 빌드 모드 그대로)
 *             CLAUDE.md "빌드 모드: dev / real" — 두 환경 데이터 절대 혼입 금지
 *
 * 목적:
 *   `initSentry()` 가 `Sentry.init({ environment })` 에 `'dev'` 또는 `'real'`
 *   문자열을 전달하는지, 그리고 `tracesSampleRate` 가 모드별로 분리되는지 회귀.
 *
 * 시나리오:
 *   M1. dev 모드  → environment='dev', tracesSampleRate=1.0
...
describe('initSentry() — environment 분리 (D-D Phase 4)', () => {
  ...
  // ─── M1 ─────────────────────────────
  it("M1: dev 모드 → environment='dev', tracesSampleRate=1.0", async () => {
  ...
  it("M3: 모듈 reset 사이 환경 분리 — 한쪽 모드 호출이 다른쪽 환경값을 오염시키지 않음", async () => {
    ...
    m.initSentry()
    const devEnv = (initSpy.mock.calls[0]?.[0] as SentryInitOpts).environment
  ...
  it('M4: DSN 누락 → dev / real 어느 쪽이든 init 호출 안 됨', async () => {
