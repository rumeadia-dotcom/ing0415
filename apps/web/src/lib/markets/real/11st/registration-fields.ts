 ...
 *
 * 11번가 카드는 [출고지 select, 반품/교환지 select] 2필드를 노출한다(11st.md §3 / §4.6 / §5).
 * 두 필드 모두 조회형(Layer 2) — useElevenStShippingAddresses 가 셀러오피스 등록분을 채운다.
 * ⚠️ officialNotice 필드는 PR-4 담당 — 본 PR(2)에서는 넣지 않는다.
 */

import { RegistrationFieldMetaSchema, type RegistrationFieldMeta } from '@/lib/schemas'

/**
 * 11번가 동적 등록필드 2개를 반환한다.
 *
 ...
 * 수집된 값(addrSeq)은 mapping.marketOptions[key] → 오케스트레이터가 addrSeqOut/addrSeqIn 으로
 * 매핑(PR-3 transformProduct 가 소비). 본 PR(2)은 select + 메타 선언까지.
 */
