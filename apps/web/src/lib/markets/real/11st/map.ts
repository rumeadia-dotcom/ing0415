...
/**
 * XML 네임스페이스 prefix 제거 (PR-0 도입, PR-6 에서 cross-market `lib/markets/xml.ts` 로 추출).
 * 11번가 REST 응답은 `ns2:categorys` / `ns2:order` / `ns2:inOutAddress` 처럼 `ns{n}:` prefix 를
 * 단다(spec import #265 — `features/11st.md` §4.3/§4.4). 매핑 로직(`pick`)이 prefix 없는 키만
 * 보게 한다. 본 파일은 기존 import 경로 호환을 위해 공용 유틸을 re-export 만 한다.
 */
...
//   내부 CarrierCode(enum) → 11번가 dlvEtprsCd 는 `lib/markets/carrier-codes.ts` 단일 소스에서
