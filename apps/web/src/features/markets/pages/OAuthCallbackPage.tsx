...
export function OAuthCallbackPage(): JSX.Element {
  ...
  if (state.kind === 'loading') {
    return (
      <div className="mx-auto w-full max-w-[640px]">
        <PageHeader
          title="OAuth 콜백 처리"
          subtitle={`${providerLabel} 인증 결과를 처리 중입니다`}
        />
        <Card>
          <CardHeader>
            <CardTitle>연결 처리 중…</CardTitle>
          </CardHeader>
          <CardContent className="py-6">
            <p className="text-sm text-text-secondary">
              {providerLabel} 토큰을 안전하게 저장하고 계정 정보를 확인하는 중입니다.
            </p>
            <div className="mt-4 flex items-center gap-2 text-sm text-text-tertiary" aria-live="polite">
              <span
                aria-hidden
  ...
  return (
    <div className="mx-auto w-full max-w-[640px]">
      <PageHeader title="연결에 실패했습니다" subtitle={`${providerLabel} 인증을 완료하지 못했습니다`} />
      <Card>
          ...
          <div className="flex gap-2">
            {isMarketId(provider) && (
              <Button asChild variant="primary">
                <Link to={`/markets/connect/${provider}`}>처음부터 다시 시도</Link>
              </Button>
            )}
