 ...
 *
 * 강제 (Wave 2 갱신, 2026-05-19):
 *   - v1 OAuth 활성 마켓 = 'naver' **단일**. 쿠팡 / G마켓 / 옥션은 비-OAuth (HMAC / ESM JWT)
 *     이므로 본 함수가 처리하지 않는다 — 호출 시 즉시 `market_not_supported` (400)
 *     + 안내 "사용 가능한 OAuth 마켓은 네이버만 — 쿠팡/G마켓/옥션은 /markets-connect 사용".
 *   - 11번가는 zod enum 단계에서 거부.
...
type SupportedMarket = 'naver'
const NON_OAUTH_MARKETS = new Set(['coupang', 'gmarket', 'auction'])

/**
 * 본 함수는 OAuth 흐름 전용. enum 은 인터페이스 호환을 위해 모든 v1 활성 마켓을 받아
 * 본문에서 명시적 차단(400) — UI 가 잘못 호출했을 때 진단 가능한 명확한 메시지 반환.
 */
...
/**
 * 네이버 OAuth authorize URL 빌드. Wave 5 에서 real 어댑터로 이관 (OQ-11).
 * 현재는 markets.md §3.1 잠정 endpoint 사용. debug 는 mock URL.
 */
...
export default Deno.serve(
  withRequest('markets-oauth-start', async ({ req, logger, correlationId }) => {
    ...
    // Wave 2: OAuth 활성 마켓은 네이버 단독. 그 외는 명시적 안내 메시지.
    ...
    const market: SupportedMarket = 'naver'
