...
# 갱신 (PUT) — 2026-05-30 부터 required 는 항상 실행되는 3개만 (무거운 잡은 CI Gate 가 대리)
...
PR 충돌 해소를 위해 main 을 release 브랜치로 머지하면 squash 가 그 머지 commit 의 일부 변경 (특히 deletion) 을 누락할 수 있다. §11 의 orphan 가드는 **선택이 아니라 의무**.
...
- ✅ WIP 핸드오프 머지 전 갱신 (§4) — 작업 진행 상태 반영, release PR 에 포함되어 main 반영
- ✅ `release/vX.Y` 브랜치 main 머지 완료 + 리모트 삭제
- ✅ `chore/backmerge-vX.Y` develop 머지 완료 + 리모트 삭제
- ✅ deploy.yml 4잡 success
- ✅ 로컬 stale 브랜치 정리

전체 단계가 사용자 승인 게이트 (§8) + 부수 점검 (§11 orphan 가드, branch protection 정합) 을 통과한 상태로 종료.
